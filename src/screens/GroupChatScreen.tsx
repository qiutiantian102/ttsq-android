...
import { Input, Button, Spinner, timeAgo, EmptyState, useRefresh } from '../components/ui';
...
export default function GroupChatScreen() {
  ...
  const emptyText = `${name || '群组'} 还没有消息`;

  const load = useCallback(async () => {
    setLoading(true);
    try { setMsgs(await api.groups.getMessages(groupId, 100)); } catch { toast.error('消息加载失败'); } finally { setLoading(false); }
  }, [groupId, toast]);
  ...
  const send = useCallback(async () => {
    ...
    try {
      await api.groups.sendMessage(groupId, { senderId: user.id, username: user.username, content: text.trim(), actorId: user.id });
      setText(''); await load();
    } catch { toast.error('发送失败'); }
  }, [user, text, groupId, load]);
